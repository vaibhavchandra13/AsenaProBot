...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        InlineKeyboardButton(strings["btn_channel"], url="https://t.me/AboutGreyWolfXD"),
        InlineKeyboardButton(
            strings["btn_group"], url="https://t.me/GreyWolfXD"
        ),
    )
    buttons.add(
        InlineKeyboardButton(
            "👸🏼 Add Asena to your group",
            url=f"https://telegram.me/asenaprobot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/AsenaProBOT?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
