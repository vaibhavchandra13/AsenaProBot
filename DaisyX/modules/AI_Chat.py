...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Asena Already Activated In This Chat")
            return
        await lel.edit(
            f"Asena Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("Asena Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Asena Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("asena", "Aco")
        test = test.replace("Asena", "Aco")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "Asena")
        response = response.replace("aco", "Asena")
...
@daisyx.on_message(
    filters.regex("Asena|asena|AsenaPro|asenapro|Asenapro")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
